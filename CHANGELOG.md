...
- Order change-window / timeline / config-reserve by parsed event time,
  ...
  January host. Yearless-only Dec/Jan pairs pin the first RFC3164 stamp
  to ``now()`` and restamp the rest against that sibling, so mid-year
  ``now()`` cannot assign both the same calendar year. An int year
  passed to ``event_time`` stays a literal calendar year.
